...
def execute_graph(request: Request, obj_in: BaseGraphSchema):
    ...
    if question.strip().lower() != 'y':  # 正常的用户提问
        events = finall_garph.stream({'messages': ('user', question)}, config, stream_mode='values')
    else:  # 用户输入的是一个： y，表示确认
        events = finall_garph.stream(None, config, stream_mode='values')
    ...
    current_state = finall_garph.get_state(config)
